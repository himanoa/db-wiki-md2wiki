...
export const toWikiText = (ctx: Context = { isOrderedList: false, listNestLevel: 0 }) => (tree: Nodes | PhrasingContent): string => {
  if(tree.type === 'text') {
    return tree.value
  }
  ...
  if(tree.type === 'listItem') {
    return addListItemModifier(`${tree.children.map(c => toWikiText(ctx)(c)).join("\n")}`, ctx.listNestLevel, ctx.isOrderedList)
  }
